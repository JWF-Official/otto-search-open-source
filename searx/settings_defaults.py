...
SEARX_ENVIRON_VARIABLES = {
    'SEARX_DISABLE_ETC_SETTINGS': 'Otto_DISABLE_ETC_SETTINGS',
    'SEARX_SETTINGS_PATH': 'Otto_SETTINGS_PATH',
    'SEARX_DEBUG': 'Otto_DEBUG',
    'SEARX_PORT': 'Otto_PORT',
    'SEARX_BIND_ADDRESS': 'Otto_BIND_ADDRESS',
    'SEARX_SECRET': 'Otto_SECRET',
}
...
SCHEMA = {
    'general': {
        'debug': SettingsValue(bool, False, 'Otto_DEBUG'),
        'instance_name': SettingsValue(str, 'Otto'),
        'privacypolicy_url': SettingsValue((None, False, str), None),
        'contact_url': SettingsValue((None, False, str), None),
        'donation_url': SettingsValue((bool, str), "https://docs.Otto.org/donate.html"),
        'enable_metrics': SettingsValue(bool, True),
    },
    'brand': {
        'issue_url': SettingsValue(str, 'https://github.com/Otto/Otto/issues'),
        'new_issue_url': SettingsValue(str, 'https://github.com/Otto/Otto/issues/new'),
        'docs_url': SettingsValue(str, 'https://docs.Otto.org'),
        'public_instances': SettingsValue(str, 'https://searx.space'),
        'wiki_url': SettingsValue(str, 'https://github.com/Otto/Otto/wiki'),
    },
    'search': {
    ...
    'server': {
        'port': SettingsValue((int, str), 8888, 'Otto_PORT'),
        'bind_address': SettingsValue(str, '127.0.0.1', 'Otto_BIND_ADDRESS'),
        'limiter': SettingsValue(bool, False),
        'secret_key': SettingsValue(str, environ_name='Otto_SECRET'),
        'base_url': SettingsValue((False, str), False, 'Otto_BASE_URL'),
    ...
    'redis': {
        'url': SettingsValue(str, 'unix:///usr/local/Otto-redis/run/redis.sock?db=0'),
    },
    'ui': {
...
def settings_set_defaults(settings):
    ...
    for searx, Otto in SEARX_ENVIRON_VARIABLES.items():
        if searx in os.environ and Otto not in os.environ:
            os.environ[Otto] = os.environ[searx]
            logger.warning('%s uses value from %s', Otto, searx)
