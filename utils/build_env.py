...
# generate the build_env, not from /etc/Otto/settings.yml.
os.environ['Otto_SETTINGS_PATH'] = join(repo_root, 'etc', 'settings.yml')
...
name_val = [
    ...
    ('Otto_URL'              , 'server.base_url'),
    ('Otto_PORT'             , 'server.port'),
    ('Otto_BIND_ADDRESS'     , 'server.bind_address'),
...
# e.g. Otto_BIND_ADDRESS (:py:obj:`searx.settings_defaults.SHEMA`).  When the
...
print('build %s (settings from: %s)' % (brand_env, os.environ['Otto_SETTINGS_PATH']))
sys.path.insert(0, repo_root)
