...
info_searx() {
    # shellcheck disable=SC1117
    cat <<EOF
To activate result and image proxy in Otto read:
  https://docs.Otto.org/admin/morty.html
Check settings in file ${Otto_SETTINGS_PATH} ...
  result_proxy:
...
install_all() {
    ...
    info_searx
    if ask_yn "Add image and result proxy to Otto settings.yml?" Yn; then
