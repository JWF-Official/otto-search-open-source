...
# The filtron target is the Otto installation, listenning on server.port at
...
# Otto suite including filtron & morty.
FILTRON_TARGET="${Otto_BIND_ADDRESS}:${Otto_PORT}"
...
APACHE_FILTRON_SITE="Otto.conf"
NGINX_FILTRON_SITE="Otto.conf"
...
install_filtron() {
    ...
    echo
    go.install github.com/Otto/filtron@latest "${SERVICE_USER}"
}
...
This installs a reverse proxy (ProxyPass) into nginx site (${NGINX_FILTRON_SITE})"
    ...
    # shellcheck disable=SC2034
    Otto_URL_PATH=$("${REPO_ROOT}/utils/searx.sh" --getenv Otto_URL_PATH)
