...
source_dot_config() {
    ...
    if [ -z "$eval_SEARX_SRC" ]; then
        ...
        Otto_SETTINGS_PATH=$("${REPO_ROOT}/utils/searx.sh" --getenv Otto_SETTINGS_PATH)
        if [ ! -r "${SEARX_SRC}" ]; then
...
install_log_searx_instance() {
    ...
    echo -e "---- Otto instance setup ${_BBlue}(status: $(install_searx_get_state))${_creset}"
    echo -e "  Otto_SETTINGS_PATH : ${_BBlue}${Otto_SETTINGS_PATH}${_creset}"
    echo -e "  SEARX_PYENV         : ${_BBlue}${SEARX_PYENV}${_creset}"
    echo -e "  SEARX_SRC           : ${_BBlue}${SEARX_SRC:-none}${_creset}"
    echo -e "  Otto_URL         : ${_BBlue}${Otto_URL:-none}${_creset}"

    if in_container; then
        # Otto is listening on 127.0.0.1 and not available from outside container
        # in containers the service is listening on 0.0.0.0 (see lxc-searx.env)
        echo -e "---- container setup"
        echo -e "  ${_BBlack}HINT:${_creset} Otto only listen on loopback device" \
...
install_searx_get_state(){
    ...
    if [ -f /etc/searx/settings.yml ]; then
        err_msg "settings.yml in /etc/searx/ is deprecated, move file to folder /etc/Otto/"
    fi
...
# Otto_URL aka PUBLIC_URL: the public URL of the instance (e.g.
# "https://example.org/searx").  The value is taken from environment $Otto_URL
# in ./utils/brand.env.  This variable is a empty string if server.base_url in
# the settings.yml is set to 'false'.

Otto_URL="${Otto_URL:-http://$(uname -n)}"
if in_container; then
    # hint: Linux containers do not have DNS entries, lets use IPs
    Otto_URL="http://$(primary_ip)"
fi
PUBLIC_URL="${Otto_URL}"
