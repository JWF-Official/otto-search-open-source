...
SEARX_INTERNAL_HTTP="${Otto_BIND_ADDRESS}:${Otto_PORT}"

Otto_URL_PATH="${Otto_URL_PATH:-$(echo "${PUBLIC_URL}" \
| sed -e 's,^.*://[^/]*\(/.*\),\1,g')}"
[[ "${Otto_URL_PATH}" == "${PUBLIC_URL}" ]] && Otto_URL_PATH=/
...
Otto_UWSGI_APP="Otto.ini"
# shellcheck disable=SC2034
SEARX_UWSGI_SOCKET="/run/uwsgi/app/Otto/socket"
...
# Apache Settings
APACHE_SEARX_SITE="Otto.conf"

# shellcheck disable=SC2034
CONFIG_FILES=(
    "${uWSGI_APPS_AVAILABLE}/${Otto_UWSGI_APP}"
)
...
install / remove
  :all:        complete (de-) installation of Otto service
  ...
  :uwsgi:      install Otto uWSGI application
  :settings:   reinstall settings from ${Otto_SETTINGS_PATH}
  :packages:   install needed packages from OS package manager
  :buildhost:  install packages from OS package manager needed by buildhosts
install
  :check:      check the Otto installation
reinstall:
  :all:        runs 'install/remove all'
update searx
  Update Otto installation ($SERVICE_HOME)
activate service
...
inspect
  ...
  :settings:   inspect YAML setting <key> from Otto instance (${SEARX_SRC})
option
  set one of the available options
apache
  :install: apache site with the Otto uwsgi app
...
main() {
    ...
    case $1 in
        ...
        install)
            ...
            case $2 in
                all)
                    rst_title "Otto (install)" part
                    install_all
                    ;;
                check)
                    rst_title "Otto (check installation)" part
                    verify_continue_install
                    install_check
                    ;;
                user)
                    rst_title "Otto (install user)"
                    verify_continue_install
                    assert_user
                    ;;
                pyenv)
                    rst_title "Otto (install pyenv)"
                    verify_continue_install
                    create_pyenv
                    ;;
                searx-src)
                    rst_title "Otto (install searx-src)"
                    verify_continue_install
                    assert_user
                    clone_searx
                    install_DOT_CONFIG
                ...
                settings)
                    ...
                    ;;
                uwsgi)
                    rst_title "Otto (install uwsgi)"
                    verify_continue_install
                    install_searx_uwsgi
                    if ! service_is_available "http://${SEARX_INTERNAL_HTTP}"; then
                        err_msg "URL http://${SEARX_INTERNAL_HTTP} not available, check Otto & uwsgi setup!"
                    fi
                    ;;
                packages)
                    rst_title "Otto (install packages)"
                    pkg_install "$SEARX_PACKAGES"
                    ;;
                buildhost)
                    rst_title "Otto (install buildhost)"
                    pkg_install "$SEARX_PACKAGES"
...
install_all() {
    rst_title "Install Otto (service)"
    verify_continue_install
    pkg_install "$SEARX_PACKAGES"
    wait_key
    assert_user
...
install_check() {
    ...
    if clone_is_available; then
        info_msg "~$SERVICE_USER: Otto software is installed."
    else
        err_msg "~$SERVICE_USER: Missing Otto software!"
    fi

    if uWSGI_app_enabled "$Otto_UWSGI_APP"; then
        info_msg "uWSGI app $Otto_UWSGI_APP is enabled."
    else
        err_msg "uWSGI app $Otto_UWSGI_APP not enabled!"
    fi

    uWSGI_app_available "$Otto_UWSGI_APP" \
        || err_msg "uWSGI app $Otto_UWSGI_APP not available!"

    sudo -H -u "${SERVICE_USER}" "${SEARX_PYENV}/bin/python" "utils/Otto_check.py"
...
update_searx() {
    rst_title "Update Otto instance"
...
EOF
    install_settings
    uWSGI_restart "$Otto_UWSGI_APP"
}

remove_all() {
    rst_title "De-Install Otto (service)"

    rst_para "\
It goes without saying that this script can only be used to remove
installations that were installed with this script."

    if ! ask_yn "Do you really want to deinstall Otto?"; then
        return
    fi
    remove_searx_uwsgi
...
clone_searx() {
    rst_title "Clone Otto sources" section
    echo
    ...
    if [[ ! "${SERVICE_HOME}" ]]; then
        err_msg "to clone Otto sources, user $SERVICE_USER hast to be created first"
        return 42
    fi
...
prompt_installation_status(){
    ...
    case $state in
        missing-searx-clone|missing-searx-pyenv)
            ...
            ;;
        *)
            info_msg "Otto instance already installed at: $SEARX_SRC"
...
prompt_installation_setting(){
    ...
    #
    # Prompts the value of the (YAML) setting in the Otto instance.
...
init_SEARX_SRC(){
    ...
    if ! clone_is_available; then
        err_msg "you have to install Otto first"
        return 1
    fi
...
install_settings() {
    rst_title "${Otto_SETTINGS_PATH}" section
    ...
    mkdir -p "$(dirname "${Otto_SETTINGS_PATH}")"
...
remove_settings() {
    rst_title "remove Otto settings" section
    echo
    info_msg "delete ${Otto_SETTINGS_PATH}"
    rm -f "${Otto_SETTINGS_PATH}"
}

remove_searx() {
    rst_title "Drop Otto sources" section
    if ask_yn "Do you really want to drop Otto sources ($SEARX_SRC)?"; then
        rm -rf "$SEARX_SRC"
    else
        rst_para "Leave Otto sources unchanged."
    fi
}
...
create_pyenv() {
    ...
    if [[ ! -f "${SEARX_SRC}/manage" ]]; then
        err_msg "to create pyenv for Otto, Otto has to be cloned first"
        return 42
    fi
...
configure_searx() {
    rst_title "Configure Otto" section
    rst_para "Setup Otto config located at $Otto_SETTINGS_PATH"
    echo
    tee_stderr 0.1 <<EOF | sudo -H -i 2>&1 |  prefix_stdout "$_service_prefix"
cd ${SEARX_SRC}
sed -i -e "s/ultrasecretkey/$(openssl rand -hex 16)/g" "$Otto_SETTINGS_PATH"
EOF
}

test_local_searx() {
    rst_title "Testing Otto instance localy" section
    echo
    ...
    sed -i -e "s/debug: false/debug: true/g" "$Otto_SETTINGS_PATH"
    tee_stderr 0.1 <<EOF | sudo -H -u "${SERVICE_USER}" -i 2>&1 |  prefix_stdout "$_service_prefix"
export Otto_SETTINGS_PATH="${Otto_SETTINGS_PATH}"
cd ${SEARX_SRC}
timeout 10 python searx/webapp.py &
sleep 3
curl --location --verbose --head --insecure $SEARX_INTERNAL_HTTP
EOF
    sed -i -e "s/debug: true/debug: false/g" "$Otto_SETTINGS_PATH"
}

install_searx_uwsgi() {
    rst_title "Install Otto's uWSGI app (Otto.ini)" section
    echo
    install_uwsgi
    uWSGI_install_app "$Otto_UWSGI_APP"
}

remove_searx_uwsgi() {
    rst_title "Remove Otto's uWSGI app (Otto.ini)" section
    echo
    uWSGI_remove_app "$Otto_UWSGI_APP"
}

activate_service() {
    rst_title "Activate Otto (service)" section
    echo
    uWSGI_enable_app "$Otto_UWSGI_APP"
    uWSGI_restart "$Otto_UWSGI_APP"
}

deactivate_service() {
    rst_title "De-Activate Otto (service)" section
    echo
    uWSGI_disable_app "$Otto_UWSGI_APP"
    uWSGI_restart "$Otto_UWSGI_APP"
}
...
enable_image_proxy() {
    ...
    tee_stderr 0.1 <<EOF | sudo -H -i 2>&1 |  prefix_stdout "$_service_prefix"
cd ${SEARX_SRC}
sed -i -e "s/image_proxy: false/image_proxy: true/g" "$Otto_SETTINGS_PATH"
EOF
    uWSGI_restart "$Otto_UWSGI_APP"
}
...
disable_image_proxy() {
    ...
    tee_stderr 0.1 <<EOF | sudo -H -i 2>&1 |  prefix_stdout "$_service_prefix"
cd ${SEARX_SRC}
sed -i -e "s/image_proxy: true/image_proxy: false/g" "$Otto_SETTINGS_PATH"
EOF
    uWSGI_restart "$Otto_UWSGI_APP"
}
...
set_result_proxy() {
    ...
    cp "${Otto_SETTINGS_PATH}" "${Otto_SETTINGS_PATH}.bak"
    _set_result_proxy "$1" "$2" > "${Otto_SETTINGS_PATH}"
}
...
EOF
    ...
    local _debug_on
    if ask_yn "Enable Otto debug mode?"; then
        enable_debug
        _debug_on=1
    fi
    echo
...
install_apache_site() {
    ...
    rst_para "\
This installs the Otto uwsgi app as apache site.  If your server is public to
...
excessively bot queries."
    ...
    rst_title "Install Otto's uWSGI app (Otto.ini)" section
    echo
    uWSGI_install_app --variant=socket "$Otto_UWSGI_APP"
...
rst-doc() {
    ...
    for DIST_NAME in ubuntu-20.04 arch fedora; do
        (
            ...
            echo -e "\n.. START Otto uwsgi-description $DIST_NAME"
...
.. code:: bash
   ...
   create     ${uWSGI_APPS_AVAILABLE}/${Otto_UWSGI_APP}
   enable:    sudo -H ln -s ${uWSGI_APPS_AVAILABLE}/${Otto_UWSGI_APP} ${uWSGI_APPS_ENABLED}/
   start:     sudo -H service uwsgi start   ${Otto_UWSGI_APP%.*}
   restart:   sudo -H service uwsgi restart ${Otto_UWSGI_APP%.*}
   stop:      sudo -H service uwsgi stop    ${Otto_UWSGI_APP%.*}
   disable:   sudo -H rm ${uWSGI_APPS_ENABLED}/${Otto_UWSGI_APP}
...
.. code:: bash
   ...
   create:    ${uWSGI_APPS_ENABLED}/${Otto_UWSGI_APP}
   enable:    sudo -H systemctl enable   uwsgi@${Otto_UWSGI_APP%.*}
   start:     sudo -H systemctl start    uwsgi@${Otto_UWSGI_APP%.*}
   restart:   sudo -H systemctl restart  uwsgi@${Otto_UWSGI_APP%.*}
   stop:      sudo -H systemctl stop     uwsgi@${Otto_UWSGI_APP%.*}
   disable:   sudo -H systemctl disable  uwsgi@${Otto_UWSGI_APP%.*}
...
.. code:: bash
   ...
   create:    ${uWSGI_APPS_ENABLED}/${Otto_UWSGI_APP}
   restart:   sudo -H touch ${uWSGI_APPS_ENABLED}/${Otto_UWSGI_APP}
   disable:   sudo -H rm ${uWSGI_APPS_ENABLED}/${Otto_UWSGI_APP}

EOF
                ;;
            esac
            echo -e ".. END Otto uwsgi-description $DIST_NAME"

            echo -e "\n.. START Otto uwsgi-appini $DIST_NAME"
            echo ".. code:: bash"
            echo
            eval "echo \"$(< "${TEMPLATES}/${uWSGI_APPS_AVAILABLE}/${Otto_UWSGI_APP}")\"" | prefix_stdout "  "
            echo -e "\n.. END Otto uwsgi-appini $DIST_NAME"
