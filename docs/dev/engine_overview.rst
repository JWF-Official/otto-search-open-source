...
Otto is a metasearch-engine_, so it uses different search engines to provide
better results.

Because there is no general search API which could be used for every search
engine, an adapter has to be built between Otto and the external search
...
It is required to tell Otto the type of results the engine provides. The
...
Otto will not start with that engine (global names with a leading underline can
be ``None``).
