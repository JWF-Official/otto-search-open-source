...
**Install** :ref:`apache Otto site` using :ref:`filtron.sh <filtron.sh overview>`

.. code:: bash

   $ sudo -H ./utils/filtron.sh apache install

**Install** :ref:`apache Otto site` using :ref:`morty.sh <morty.sh overview>`
...
modules and create a `Location`_ configuration for the Otto site.  In most
...
.. tabs::
   ...
   .. group-tab:: Ubuntu / debian
      ...
      In :ref:`The Debian Layout` you create a ``Otto.conf`` with the
      ``<Location /searx >`` directive and save this file in the *sites
      available* folder at ``/etc/apache2/sites-available``.  To enable the
      ``Otto.conf`` use :man:`a2ensite`:

      .. code:: sh

         sudo -H a2ensite Otto.conf
...
.. _apache Otto via filtron plus morty:
...
package and you need to install only the :ref:`uWSGI <Otto uwsgi>` package:
...
Otto account (see :ref:`create Otto user`):
