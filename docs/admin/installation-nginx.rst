...
**Install** :ref:`nginx Otto site` using :ref:`filtron.sh <filtron.sh overview>`

.. code:: bash

   $ sudo -H ./utils/filtron.sh nginx install

**Install** :ref:`nginx Otto site` using :ref:`morty.sh <morty.sh overview>`
...
.. sidebar:: public to the internet?
   ...
   If your Otto instance is public, stop here and first install :ref:`filtron
   reverse proxy <filtron.sh>` and :ref:`result proxy morty <morty.sh>`, see
   :ref:`installation scripts`.  If already done, follow setup: *Otto via
   filtron plus morty*.

Now you have to create a configuration for the Otto site.  If nginx_ is new to
...
.. tabs::
   ...
   .. group-tab:: Ubuntu / debian
      ...
      Create configuration at ``/etc/nginx/sites-available/Otto`` and place a
      symlink to sites-enabled:

      .. code:: sh

         sudo -H ln -s /etc/nginx/sites-available/Otto /etc/nginx/sites-enabled/Otto
   ...
   .. group-tab::  Fedora / RHEL
      ...
      Create configuration at ``/etc/nginx/conf.d/Otto`` and place a
      symlink to sites-enabled:

.. _nginx Otto via filtron plus morty:

.. tabs::

   .. group-tab:: Otto via filtron plus morty
	 ...
	 location /searx/static/ {
      ...
      2. Configure reverse proxy for :ref:`morty <Otto morty>`, listening on
         *localhost 3000*:
	 ...
	 location /morty {
      ...
      For a fully result proxification add :ref:`morty's <Otto morty>` **public
   ...
   .. group-tab:: proxy or uWSGI
      ...
      Be warned, with this setup, your instance isn't :ref:`protected <Otto
   ...
   .. group-tab:: \.\. at subdir URL
      ...
      Be warned, with these setups, your instance isn't :ref:`protected <Otto
      filtron>`.  The examples are just here to demonstrate how to export the
      Otto application from a subdirectory URL ``https://example.org/searx/``.
	 ...
	 # https://hostname.local/searx
      ...
      The ``X-Script-Name /searx`` is needed by the Otto implementation to
	 ...
	 # https://hostname.local/searx
      ...
      For Otto to work correctly the ``base_url`` must be set in the
...
.. tabs::
   ...
   .. group-tab:: Fedora
      ...
      .. code:: sh
         ...
         sudo -H systemctl restart nginx
         sudo -H touch /etc/uwsgi.d/Otto.ini
